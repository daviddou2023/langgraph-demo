...
# 4. 通过指定提供商和模型名称进行初始化
llm = init_chat_model(
    model="deepseek-v4-flash", # 这里以您的配置为准
...
class BasicToolNode:
    ...
    def __call__(self, inputs: dict):
        ...
        for tool_call in message.tool_calls:
            # 这里调用 tool_result 时如果遇到 human_assistance，就会触发 interrupt
...
# ==================== 运行与交互循环 (已升级支持 Interrupt) ====================

def run_interactive_loop():
    """交互式终端对话逻辑（支持记忆与人工干预）。"""
    print("\n--- 进入实时交互模式 (输入 'q' 退出) ---")
    print("测试建议：你可以对模型说：'我有个秘密问题，请使用 human_assistance 工具向人类求助。'")
    
    while True:
        try:
            # 1. 每次循环前，先检查图的状态，看是否处于挂起 (Interrupted) 状态
            state = graph.get_state(config)
            
            # 如果存在任务，且任务列表中存在 interrupts，说明触发了求助
            if state.tasks and state.tasks[0].interrupts:
                ...
                # 【核心修改点 2】：通过 Command 恢复状态机的执行，并传入人工回复的数据
